...
MECH_RUNS = frozenset({"run27", "run28", "run29", "run30", "run31"})
...
NARRATIVE: dict[str, tuple[str, str]] = {
    ...
    "run30": (
        ...
        (
            ...
            "判决仍是 16 null、4 blocked。因此 run30 不是「修复改变了判决」的实证，"
            "改变判决的实证在回溯投影那一节。run30 给出的是另一种实证："
            "本轮读数最大的三条符号全部与所声明的方向相反，第一条还越过本轮证否线；"
            "它们若出现在没有品种维退化这道闸的面板 universe 上（run29-study-4 正是如此），"
            "方向核对就是唯一拦得住它们的判据。越线且符号一致者：无。"
        ),
    ),
    "run28": (
        "run28 · 机制先验族的第一轮完整检验",
        (
            "决定 0011 的新账 sc_mechanism_prior_v1 首次跑满。十九版判决、"
            "统计分母十五：十三 null、五 blocked、一 candidate。"
            ...
            "不是「效应大不大」。机械失败 0 次解析失败、0 次 provider 修复，"
            "对照 run26 的四分之一损耗；唯一一次上下文拦截来自修复提示词回显了"
            "模型自己写的 ic_dominant_t1，撞上效果字段前缀 ic_（已修）。"
        ),
    ),
    "run31": (
        "run31 · 覆盖全品种，与三条方向一致的候选",
        (
            "决定 0013 的四十票：新增机制子面板形态、第二批三条机制轴，"
            "证否线由 E2(45)=2.4933 抬到 **E2(82)=2.7000**（覆盖全品种的真实代价："
            "多买四十张彩票，中奖标准就要相应提高）。提案**全部**使用子面板，"
            "覆盖品种数由上一轮的十几个升到四十个；未被读到的品种里，"
            "除极少数外全部是目标表行数不足者，即在「有足够历史可供检验」这个意义上，"
            "覆盖已经完成。本轮的候选与 run29 的那三条性质不同：方向判据已经生效，"
            "因此它们的符号与自己声明的方向一致；但三条的 |t| 全部低于各自读取时刻的"
            "本族地板，属「闸门全过而效应微弱」，不是发现。另有一条检验的 t 因双向 "
            "cluster 方差为负而无定义，评价机如实判 null 并记下理由，没有改用替代"
            "标准误去凑一个数 —— 报告据此把「因 cluster 结构不足而无定义」单列计数，"
            "同时分母照扣。"
        ),
    ),
    "run26": (
