"""auto_flow_recent21600_flow_base345600_attention
...
规格内容身份 4e2ed5ae7cb0554750862bfb9ff4f9c309edd3e94df493e6a2e4d77ebc982992
生成器 0.1.0；改规格才能改代码，两者不可能不一致。

PIT：每个窗口的右端点一律**不含**决策时点，offset 只能非负。
无定义返回 None，绝不返回 0 —— 返回 0 会被下游当成「没有信号」，那是伪造。
本特征最早触及 4日 前的数据；该深度未整段落在序列覆盖范围内时判为无定义。
"""
...
REQUIRED_LOOKBACK_SECONDS = 345600
...
def compute(decision_time: datetime, series: dict, coverage_start=None):
    ...
    v_flow_base = _agg("sum", _window(t_flow_base, x_flow_base, end_flow_base, 345600))
