# SC Temporal Spine 人工回放清单（sc_ret_next_session）

- 抽样种子：`20260805`（按样本段分层，结果可复现）
- 样本量：30
- spine 指纹：`6cdb82e3ddc25ad01ee67e48f62fa551cead8d8ce51c81c079ae14a82a27881a`
...
- 目标值：-0.023272782 = ln(505.4 / 517.3)
...
- 目标值：0.014539687 = ln(547.3 / 539.4)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.015621204 = ln(574.2 / 565.3)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.0054421903 = ln(515.9 / 513.1)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.021725357 = ln(514.5 / 525.8)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.0045930165 = ln(586.5 / 589.2)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.0001696209 = ln(589.6 / 589.5)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.0084464515 = ln(554.1 / 558.8)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.0016485332 = ln(606.1 / 607.1)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.00077609627 = ln(644.5 / 644)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.00065941315 = ln(606.8 / 606.4)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.00031933578 = ln(626.2 / 626.4)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.012017312 = ln(527.4 / 521.1)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.00072780207 = ln(549.8 / 549.4)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.013854023 = ln(617.8 / 609.3)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.012067469 = ln(469.5 / 475.2)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.005272089 = ln(532.5 / 529.7)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.0010238559 = ln(488.1 / 488.6)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.011798914 = ln(468.9 / 463.4)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.0051060162 = ln(451.6 / 449.3)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.0031638444 = ln(441.8 / 443.2)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.0012006245 = ln(416.2 / 416.7)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：-0.003866717 = ln(438.8 / 440.5)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.023723131 = ln(605.7 / 591.5)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
...
- 目标值：0.037726159 = ln(542.9 / 522.8)
  - 请人工核对：入场价是否为**执行时点那一分钟最后一笔**的收盘，出场价是否为 session 最后一根 bar 的收盘，两者是否都不在涨跌停上
